...
for K in [1,2,3,4]:
	...
	for i in xrange(len(LL)-1):
		assert LL[i] >= LL[i+1], "Log-Likelihoods not monotonically increasing!"
	p3.plot2D(X,K,Mu,P,Var,post,"plot for K = %d" % K)
...
# X = p3.readData('netflix_incomplete.txt')
# Xtrue = p3.readData('netflix_complete.txt')
# K = 12
# best_LL = -1e18
# Mu_best = None
# P_best = None
# Var_best = None
# for num_rounds in xrange(10):
# 	(Mu, P, Var) = p3.init(X,K)
# 	(Mu,P,Var,post,LL) = p3.mixGauss_part2(X,K,Mu,P,Var)
# 	if LL[0] > best_LL:
# 		best_LL = LL[0]
...
# Xpred = p3.fillMatrix(X, K, Mu_best, P_best, Var_best)
# print "RMSE:", p3.rmse(Xpred, Xtrue)
